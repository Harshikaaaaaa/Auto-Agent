...
import { isMysqlAvailable, loadDbModules, restoreEnv, truncateAll } from '../../test/dbTestHelper.js';
...
beforeAll(async () => {
    ...
    const modules = await loadDbModules(vi, { DB_NAME: `${process.env.TEST_DB_NAME ?? 'autoagent_test'}_cred` });
    pool = modules.pool;
...
describe.skipIf(!mysqlAvailable)('credentialRepository', () => {
    ...
    it('is one row per owner and tool', async () => {
        ...
        const [rows] = await queryRaw('SELECT tool_id FROM tool_credentials WHERE owner_id = ?', [OWNER]);
