...
describe('rejects tampering and misuse', () => {
    ...
    it('cannot be decrypted with a different secret', async () => {
        const first = await loadCrypto({ CREDENTIAL_SECRET: 'secret-number-one-padded-to-32-characters' });
        const sealed = first.encryptSecret('token');

        const second = await loadCrypto({ CREDENTIAL_SECRET: 'secret-number-two-padded-to-32-characters' });
