...
describe('authorization challenge', () => {
    ...
    it('refuses an unknown tool', async () => {
        ...
        expect(() => buildAuthorizationUrl({ toolId: 'dropbox', codeChallenge: 'c', state: 's' })).toThrow(
            OAuthError,
        );
    });
});
...
describe('getAccessToken', () => {
    ...
    it('reports not_connected when there is no credential', async () => {
        const { getAccessToken } = await loadGoogle();
        await expect(getAccessToken('operator', 'gmail', { fetchImpl: tokenFetch({}) })).rejects.toMatchObject(
            { reason: 'not_connected', status: 401 },
        );
    });
    ...
    it('reports refresh_unavailable rather than sending an expired token', async () => {
        ...
        await expect(getAccessToken('operator', 'gmail', { fetchImpl: tokenFetch({}) })).rejects.toMatchObject(
            { reason: 'refresh_unavailable', status: 401 },
        );
    });
});
