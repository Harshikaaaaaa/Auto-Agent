export const AI_CONFIG = {
  ...
  getRecommendedModel(
    prompt: string = '',
    preferredProvider?: 'ollama' | 'gemini' | 'openrouter' | 'auto',
  ) {
    if (preferredProvider && preferredProvider !== 'auto') {
      return preferredProvider === 'ollama'
        ? this.ollamaModel
  ...
  getProviderHeuristic(prompt: string = '') {
    ...
    if (/analyze|research|compare|plan|architecture|complex|reason/.test(normalized))
      return 'openrouter';
    return 'gemini';
  },
};
