...
describe('applying operations', () => {
  ...
  it('gives a unique id to a node whose label collides with an existing id', () => {
    ...
    expect(result.nodes.map((n) => n.id)).toEqual(['start', 'start_2', 'fetch', 'save_sheets']);
  });
  ...
  it('adds an edge once', () => {
    ...
    expect(
      result.edges.filter((e) => e.source === 'start' && e.target === 'save_sheets'),
    ).toHaveLength(1);
  });
...
describe('destructive classification', () => {
  ...
  it('treats additive and cosmetic operations as safe', () => {
    expect(isDestructive(patch({ op: 'addEdge', source: 'a', target: 'b', condition: null }))).toBe(
      false,
    );
...
describe('describeOperation', () => {
  it('names steps by label, not by id', () => {
    expect(describeOperation({ op: 'removeNode', nodeId: 'save_sheets' }, SHEETS_GRAPH.nodes)).toBe(
      'Remove “Save results to Google Sheets”',
    );

    expect(
      describeOperation({ op: 'reorder', nodeIds: ['save_sheets', 'fetch'] }, SHEETS_GRAPH.nodes),
