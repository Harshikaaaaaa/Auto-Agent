...
describe('callGoogle', () => {
    ...
    it('maps a 401 to a needsReconnect error', async () => {
        vi.stubGlobal(
            'fetch',
            vi.fn(async () => jsonResponse({ error: 'google_auth_failed', message: 'Reconnect the tool.' }, 401))
        );
    ...
    it('treats not_connected as needing reconnect regardless of status', async () => {
        vi.stubGlobal(
            'fetch',
            vi.fn(async () => jsonResponse({ error: 'not_connected', message: 'Connect it first.' }, 401))
        );
