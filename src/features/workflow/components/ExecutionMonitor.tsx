...
import { X, Terminal as TerminalIcon, Download, ChevronDown, ChevronRight, CheckCircle2, Circle, Loader2, AlertCircle } from 'lucide-react';
...
export const ExecutionMonitor: React.FC<ExecutionMonitorProps> = ({
    ...
    const failureCount = nodeFeedback.filter(item => item.status === 'failed' || item.status === 'retrying').length;
        ...
        <div className="w-[520px] glass-panel rounded-3xl shadow-2xl flex flex-col h-[560px] overflow-hidden border-white/10">
            <div className="p-5 border-b border-white/10 flex items-center justify-between">
                <div className="flex items-center gap-3">
                    ...
                    {graphState && (
                        <span className={`text-[9px] px-2 py-0.5 rounded-full font-bold ${graphState.__metadata.status === 'completed'
            ...
            {isExecuting && currentNodeLabel && (
                <div className="px-5 py-3 border-b border-white/10 bg-white/5 text-[11px] text-white/80 flex items-center justify-between gap-3">
                    ...
                    <span className="rounded-full bg-bolt-accent/15 px-3 py-1 text-[10px] uppercase tracking-[0.18em] text-bolt-accent">{currentNodeLabel}</span>
                </div>
            )}
            ...
            <div className="px-5 py-3 border-b border-white/10 bg-[#0a0a0a]">
                <div className="flex gap-2 flex-wrap text-[9px] font-bold uppercase tracking-[0.16em] text-white/60">
                    <span className="rounded-full border border-white/10 bg-white/5 px-2 py-1">Status: {latestStatus}</span>
                    <span className="rounded-full border border-white/10 bg-white/5 px-2 py-1">Nodes: {nodeCount}</span>
                    <span className="rounded-full border border-white/10 bg-white/5 px-2 py-1">Failures: {failureCount}</span>
                </div>
            </div>
                        ...
                        <div key={idx} className="space-y-2 group">
                            <div className="flex justify-between items-center border-b border-white/5 pb-1">
                                <span className="text-bolt-accent font-bold flex items-center gap-2">
                                    ...
                                    {status === 'running' && <Loader2 className="w-3 h-3 animate-spin text-bolt-accent" />}
                                    {status === 'pending' && <Circle className="w-3 h-3 text-white/25" />}
                                    {isFailed && <AlertCircle className="w-3 h-3 text-red-400" />}
                                    <span className="text-white/30">[{idx + 1}/{nodeCount}]</span>{' '}
