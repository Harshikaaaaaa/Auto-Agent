            ...
            <div className="bg-[#0a0a0a] border border-white/10 rounded-2xl w-full max-w-2xl shadow-2xl overflow-hidden">
                ...
                <div className="p-6 max-h-[60vh] overflow-y-auto space-y-4">
                        ...
                        >
                            <div className="flex items-center gap-2">
                                ...
                                <h4 className="text-sm font-semibold text-amber-200">
                                    {unsupported.length}{' '}
                                    {unsupported.length === 1 ? 'step has' : 'steps have'} no tool
                                    behind {unsupported.length === 1 ? 'it' : 'them'}
                                </h4>
                            </div>
                            ...
                            <p className="mt-2 text-[11px] text-amber-100/60">
                                A run will stop at the first of these. Delete the step, or connect a
                                tool that can do it and generate again.
                            </p>
                        </div>
                    )}
                    ...
                    {missingInputs.length > 0 && (
                        <div className="rounded-xl border border-sky-500/30 bg-sky-500/10 p-4">
                            <h4 className="text-sm font-semibold text-sky-200">
                                ...
                                {missingInputs.length === 1 ? 'value is' : 'values are'} still needed
                            </h4>
                            <p className="mt-1 text-[11px] text-sky-100/70">
                                No earlier step produces {missingInputs.length === 1 ? 'it' : 'them'}
                                , so you have to supply {missingInputs.length === 1 ? 'it' : 'them'}.
                                Fill them in below, or later on the canvas.
                            </p>
                        </div>
                    )}
                    ...
                    {unconnectedTools.length > 0 && (
                        <div className="rounded-xl border border-white/10 bg-white/5 p-4">
                            ...
                            <p className="mt-1 text-[11px] text-white/50">
                                {unconnectedTools.map(tool => tool.name).join(', ')} — the run will
                                stop until you connect {unconnectedTools.length === 1 ? 'it' : 'them'}.
                            </p>
                        </div>
                    )}
                    ...
                    {nodes.map((node, index) => {
                        const status =
                            statuses[node.id] ?? (index < committedCount ? 'done' : 'pending');
                                    ...
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <div className="mt-2 flex flex-wrap items-center gap-1.5">
                                            <span className="rounded-full border border-white/10 bg-black/30 px-2 py-0.5 text-[9px] font-bold uppercase tracking-[0.12em] text-white/50">
                                                {TYPE_LABELS[String(node.data.type)] ??
