                    ...
                    workflow:
                        task.workflow && Array.isArray(task.workflow.nodes)
                            ? {
                                  ...
                                  edges: Array.isArray(task.workflow.edges)
                                      ? (task.workflow.edges as Edge[])
                                      : []
                              }
                            : { nodes: [], edges: [] }
                };
...
export function isMeaningfulWorkflowPrompt(prompt: string): PromptCheck {
    ...
    const workflowSignals = [
        'email', 'gmail', 'slack', 'sheet', 'google sheets', 'drive', 'whatsapp', 'notify', 'save',
        'send', 'summarize', 'update', 'create', 'sync', 'automate', 'workflow', 'trigger', 'approval',
        'capture', 'log', 'archive', 'manage', 'monitor', 'follow up', 'lead'
    ];
    ...
    if (!hasSignal && !hasActionVerb) {
        return {
            valid: false,
            reason: 'I can only turn real workflow goals into plans. Please describe a concrete automation task.'
        };
    }
