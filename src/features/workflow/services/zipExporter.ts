...
function envFileContent(vars: string[], extra = ''): string {
  const lines = vars.map((v) => `${v}=your_${v.toLowerCase()}_here`).join('\n');
  return `# Environment variables - DO NOT commit this file\n${lines}${extra ? '\n' + extra : ''}\n`;
}

function readmeContent(framework: string, projectName: string, description: string, usage: string): string {
  return `# ${projectName}\n\n> Generated by **AutoAgent** · Framework: **${framework}**\n\n## Description\n${description}\n\n## Quick Start\n\n\`\`\`bash\n# 1. Create & activate a virtual environment\npython -m venv .venv\nsource .venv/bin/activate   # Windows: .venv\\Scripts\\activate\n\n# 2. Install dependencies\npip install -r requirements.txt\n\n# 3. Configure environment\ncp .env.example .env\n# Edit .env and fill in your API keys\n\n# 4. Run\n${usage}\n\`\`\`\n\n---\n*AutoAgent - AI Workflow Orchestration Platform*\n`;
}
...
//     __init__.py
export function buildGoogleADKZip(ctx: WorkflowExportContext, code: string): ZipProjectTemplate {
  return {
    rootFolder: 'my_agent',
    files: {
      'agent.py': code,
